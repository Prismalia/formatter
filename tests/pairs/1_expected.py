...
def does_file_contain_tripadvisor(path):
  ...
  try:
    return (
      df["Hotel Name"]
      .map(lambda x: "tripadvisor" in x.lower() if isinstance(x, str) else False)
      .any()
    )
  except:
    return False
...
tripadvisored_output_files = [
  f for f in output_files if does_file_contain_tripadvisor(f)
]
tripadvisored_output_files = [
  e.parent / "hotels.csv" for e in tripadvisored_output_files
]
print(
  f"{len(tripadvisored_output_files)} tripadvisored output files out of {len(output_files)}"
)
